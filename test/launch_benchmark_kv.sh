...
BM_DEF_SERVER_EXTA_ARGS="--swap-space 16 --gpu-memory-utilization 0.92 --dtype auto --max-num-seqs 32 --disable-log-requests"
...
function run() {
    ...
    if [ x"$BM_IMAGE" != x"" ] && [ x"$BM_MODEL_DIR" != x"" ]; then
        ...
        docker_args="-d --gpus all --privileged --ipc=host --net=host -v $BM_MODEL_DIR:/this_model -e CUDA_VISIBLE_DEVICES=$BM_GPU_IDS -e FULL_KV_LAYERS=6"
