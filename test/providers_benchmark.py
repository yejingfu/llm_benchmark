...
def run_benchmark(provider: LlmProvider, contexts: List[Context], tokenizer: AutoTokenizer, args: argparse.Namespace):
    ...
    for ctx in contexts:
        ctx.clean()
    extra = {}
    sender = AysncRequestSender(provider.endpoint, provider.model, provider.api_key, None, True, True, args.print_verbose)
    ...
    output += f"num erros: {metrics.num_errors}\n"
    print(output)
    for ctx in contexts:
...
def main(args: argparse.Namespace):
        ...
        with open(config_file, "r") as f:
            ...
            for p in json_data["providers"]:
                ...
                for n in p["model-names"]:
                    ...
                    providers.append(LlmProvider(p["name"], n, ep2, p["api_key"]))
    if len(providers) == 0:
    ...
    norm_prompt_lens = np.rint(np.random.normal(args.input_len, 20, size=args.num_requests)).astype(np.int32)
    norm_output_lens = np.rint(np.random.normal(args.output_len, 10, size=args.num_requests)).astype(np.int32)
